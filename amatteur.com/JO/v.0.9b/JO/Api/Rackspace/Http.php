...
class JO_Api_Rackspace_Http {
	
	const PHP_CF_VERSION 					= '1.7.11';
	const USER_AGENT 						= 'PHP-CloudFiles/1.7.11';
	...
	const ACCOUNT_KEY 						= 'X-Account-Meta-Key';
	const ACCOUNT_METADATA_HEADER_PREFIX 	= 'X-Account-Meta-';
	...
	const DELETE_AFTER 						= 'X-Delete-After';
	const DELETE_AT 						= 'X-Delete-At';
    ...
    function ssl_use_cabundle($path=NULL)
    {
        ...
        if (!file_exists($this->cabundle_path)) {
            throw new JO_Api_Rackspace_Exception_Io("Could not use CA bundle: "
                . $this->cabundle_path);
        }
        return;
    }
    ...
    {
        if(!$path)
            throw new JO_Api_Rackspace_Exception_Syntax("Path not set");
    ...
    {
        if (!is_object($obj) || get_class($obj) != "JO_Api_Rackspace_Object") {
            throw new JO_Api_Rackspace_Exception_Syntax(
                "Method argument is not a valid JO_Api_Rackspace_Object.");
        }
    ...
    function post_account(&$conn)
    {
        ...
        if (!is_array($conn->metadata)) {
            throw new JO_Api_Rackspace_Exception_Syntax("Metadata array is empty");
        }
    ...
    function copy_object($src_obj_name, $dest_obj_name, $container_name_source, $container_name_target, $metadata=NULL, $headers=NULL)
    {
        ...
        $hdrs[self::DESTINATION] = $destination;
    ...
    private function _header_cb($ch, $header)
    {
        ...
        case strtolower(self::ORIGIN_HEADER):
            ...
            break;
        case strtolower(self::ACCOUNT_KEY):
        ...
        default:
            ...
            elseif (strncasecmp($name, self::ACCOUNT_METADATA_HEADER_PREFIX, strlen(self::ACCOUNT_METADATA_HEADER_PREFIX)) == 0) {
                $this->_account_metadata[$name] = $value;
            }
            elseif (strncasecmp($name, self::CONTAINER_METADATA_HEADER_PREFIX, strlen(self::CONTAINER_METADATA_HEADER_PREFIX)) == 0) {
    ...
    private function _process_headers($metadata=null, $headers=null)
    {
        ...
        for ($argi = 0; $argi < $argc; $argi++) {
            ...
            foreach ($argv[$argi] as $k => $v) {
                $k = trim($k);
                $v = trim($v);
                if (strpos($k, ":") !== False) throw new JO_Api_Rackspace_Exception_Syntax(
                ...
                if (array_key_exists('filter', $rule)) {
                    ...
                    if (!$result) throw new JO_Api_Rackspace_Exception_Syntax(sprintf(
    ...
    private function _send_request($conn_type, $url_path, $hdrs=NULL, $method="GET", $force_new=False)
    {
        ...
        if (gettype($this->connections[$conn_type]) == "unknown type")
            throw new JO_Api_Rackspace_Exception_Connectionnotopen (
