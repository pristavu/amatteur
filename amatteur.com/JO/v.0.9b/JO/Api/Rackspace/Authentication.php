 ...
 * #
 * # $auth = new JO_Api_Rackspace_Authentication("username, "api_key", NULL, JO_Api_Rackspace_Authentication::UK_AUTHURL);
 * # Using the JO_Api_Rackspace_Authentication::UK_AUTHURL keyword will force the api to use the UK AuthUrl.
 ...
 */
class JO_Api_Rackspace_Authentication {
	 ...
	 */
	function __construct($username=NULL, $api_key=NULL, $account=NULL, $auth_host=JO_Api_Rackspace_Authentication::US_AUTHURL)
	{
	 ...
	 * @throws JO_Api_Rackspace_Exception_Authentication invalid credentials
	 * @throws JO_Api_Rackspace_Exception_Invalidresponse invalid response
	 */
	function authenticate($version=JO_Api_Rackspace_Authentication::DEFAULT_CF_API_VERSION)
	{
		...
		if ($status == 401) {
			throw new JO_Api_Rackspace_Exception_Authentication("Invalid username or access key.");
		}
	 ...
	 * @throws JO_Api_Rackspace_Exception_Syntax If any of the Required Arguments are missing
	 */
	function load_cached_credentials($auth_token, $storage_url, $cdnm_url)
	{
		if(!$storage_url || !$cdnm_url)
		{
			throw new JO_Api_Rackspace_Exception_Syntax("Missing Required Interface URL's!");
			return False;
		}
		if(!$auth_token)
		{
			throw new JO_Api_Rackspace_Exception_Syntax("Missing Auth Token!");
			return False;
		}
	...
	/**
	 * Make sure the JO_Api_Rackspace_Authentication instance has authenticated.
	 *
