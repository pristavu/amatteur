...
class JO_Api_Rackspace_Object {
	...
	function __construct(&$container, $name, $force_exists=False, $dohead=True)
	{
		...
		if (strlen($name) > JO_Api_Rackspace_Authentication::MAX_OBJECT_NAME_LEN) {
			throw new JO_Api_Rackspace_Exception_Syntax("Object name exceeds "
		...
		if ($dohead) {
			if (!$this->_initialize() && $force_exists) {
				throw new JO_Api_Rackspace_Exception_Nosuchobject("No such object '".$name."'");
			}
		}
	}
	...
	function _guess_content_type($handle) {
		...
		if(!$this->content_type) {
			$this->content_type = JO_File_Ext::getMimeFromFile($handle);
		}
	
		if (!$this->content_type) {
			throw new JO_Api_Rackspace_Exception_Badcontenttype("Required Content-Type not set");
		}
		return True;
	}
	...
	function read($hdrs=array())
	{
		...
		if (($status < 200) || ($status > 299
			...
			throw new JO_Api_Rackspace_Exception_Invalidresponse("Invalid response (".$status."): "
	 ...
	 * @throws JO_Api_Rackspace_Exception_Syntax missing required parameters
	 * @throws JO_Api_Rackspace_Exception_Badcontenttype if no Content-Type was/could be set
	 * @throws JO_Api_Rackspace_Exception_Mismatchedchecksum $verify is set and checksums unequal
	 * @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
	 */
	function write($data=NULL, $bytes=0, $verify=True)
	{
		if (!$data && !is_string($data)) {
			throw new JO_Api_Rackspace_Exception_Syntax("Missing data source.");
		}
		if ($bytes > JO_Api_Rackspace_Authentication::MAX_OBJECT_SIZE) {
			throw new JO_Api_Rackspace_Exception_Syntax("Bytes exceeds maximum object size.");
		}
		if ($verify) {
		...
		if (!is_resource($data)) {
			...
			if ($this->content_length > JO_Api_Rackspace_Authentication::MAX_OBJECT_SIZE) {
				throw new JO_Api_Rackspace_Exception_Syntax("Data exceeds maximum object size");
			}
		...
		if ($status == 412) {
			...
			throw new JO_Api_Rackspace_Exception_Syntax("Missing Content-Type header");
		}
		if ($status == 422) {
			if ($close_fh) {
				fclose($fp);
			}
			throw new JO_Api_Rackspace_Exception_Mismatchedchecksum(
	 ...
	 * @throws JO_Api_Rackspace_Exception_Mismatchedchecksum $verify is set and checksums unequal
	 * @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
	 * @throws JO_Api_Rackspace_Exception_Io error opening file
	 */
	function load_from_filename($filename, $verify=True)
	{
		$fp = @fopen($filename, "r");
		if (!$fp) {
			throw new JO_Api_Rackspace_Exception_Io("Could not open file for reading: ".$filename);
		}
	
		clearstatcache();
	
		$size = (float) sprintf("%u", filesize($filename));
		if ($size > JO_Api_Rackspace_Authentication::MAX_OBJECT_SIZE) {
			throw new JO_Api_Rackspace_Exception_Syntax("File size exceeds maximum object size.");
		}
	...
	function save_to_filename($filename)
	{
		...
		if (!$fp) {
			throw new JO_Api_Rackspace_Exception_Io("Could not open file for writing: ".$filename);
		}
	 ...
	 * @throws JO_Api_Rackspace_Exception_Cdnnotenabled if CDN Is not enabled on this connection
	 * @throws JO_Api_Rackspace_Exception_Invalidresponse if the response expected is not returned
	 */
	function purge_from_cdn($email=null)
	{
		if (!$this->container->cfs_http->getCDNMUrl())
		{
			throw new JO_Api_Rackspace_Exception_Cdnnotenabled(
