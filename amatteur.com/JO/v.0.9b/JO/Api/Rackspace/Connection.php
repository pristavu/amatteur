 ...
 * #
 * $auth = new JO_Api_Rackspace_Authentication("username", "api_key");
 *
 ...
 * # validated JO_Api_Rackspace_Authentication instance.
 * #
 ...
 * #       call the JO_Api_Rackspace_Authentication instance's 'ssl_use_cabundle()' method.
 * #
 ...
 */
class JO_Api_Rackspace_Connection {
		...
		/**
		 * Pass in a previously authenticated JO_Api_Rackspace_Authentication instance.
		 *
		 * Example:
		 * <code>
		 * # Create the authentication instance
		 ...
		 *
		 * @param obj $cfs_auth previously authenticated JO_Api_Rackspace_Authentication instance
		 * @param boolean $servicenet enable/disable access via Rackspace servicenet.
		 * @throws JO_Api_Rackspace_Exception_Authentication not authenticated
		 */
		function __construct($cfs_auth, $servicenet=False)
		{
			if (isset($_ENV['RACKSPACE_SERVICENET']))
				$servicenet=True;
			$this->cfs_http = new JO_Api_Rackspace_Http(JO_Api_Rackspace_Authentication::DEFAULT_CF_API_VERSION);
			$this->cfs_auth = $cfs_auth;
			if (!$this->cfs_auth->authenticated()) {
				$e = "Need to pass in a previously authenticated ";
				$e .= "JO_Api_Rackspace_Authentication instance.";
				throw new JO_Api_Rackspace_Exception_Authentication($e);
			}
		 ...
		 * @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
		 */
		function get_info()
		{
		 ...
		 * @return JO_Api_Rackspace_Container
		 * @throws JO_Api_Rackspace_Exception_Syntax invalid name
		 * @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
		 */
		function create_container($container_name=NULL)
		{
			if ($container_name != "0" and !isset($container_name))
				throw new JO_Api_Rackspace_Exception_Syntax("Container name not set.");
			...
			if (strpos($container_name, "/") !== False) {
				...
				throw new JO_Api_Rackspace_Exception_Syntax($r);
			}
			if (strlen($container_name) > JO_Api_Rackspace_Authentication::MAX_CONTAINER_NAME_LEN) {
				throw new JO_Api_Rackspace_Exception_Syntax(sprintf(
						"Container name exeeds %d bytes.",
						JO_Api_Rackspace_Authentication::MAX_CONTAINER_NAME_LEN));
			}
	
			$return_code = $this->cfs_http->create_container($container_name);
			if (!$return_code) {
				throw new JO_Api_Rackspace_Exception_Invalidresponse("Invalid response ("
			...
			return new JO_Api_Rackspace_Container($this->cfs_auth, $this->cfs_http, $container_name);
		}
		...
		* @throws JO_Api_Rackspace_Exception_Syntax missing proper argument
		* @throws JO_Api_Rackspace_Exception_Invalidresponse invalid response
		* @throws JO_Api_Rackspace_Exception_Nonemptycontainer container not empty
		* @throws JO_Api_Rackspace_Exception_Nosuchcontainer remote container does not exist
		*/
		...
		if (get_class($container) == "JO_Api_Rackspace_Container") {
	...
	if ($container_name != "0" and !isset($container_name))
		throw new JO_Api_Rackspace_Exception_Syntax("Must specify container object or name.");
	
		$return_code = $this->cfs_http->delete_container($container_name);
	
		if (!$return_code) {
		throw new JO_Api_Rackspace_Exception_Invalidresponse("Failed to obtain http response");
	}
		#if ($status == 401 && $this->_re_auth()) {
		#    return $this->delete_container($container);
		#}
		if ($return_code == 409) {
		throw new JO_Api_Rackspace_Exception_Nonemptycontainer(
		"Container must be empty prior to removing it.");
		}
			if ($return_code == 404) {
			throw new JO_Api_Rackspace_Exception_Nosuchcontainer(
			...
			* @return container JO_Api_Rackspace_Container instance
			* @throws JO_Api_Rackspace_Exception_Nosuchcontainer thrown if no remote Container
			* @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
			*/
			...
			#}
			if ($status == 404) {
			throw new JO_Api_Rackspace_Exception_Nosuchcontainer("Container not found.");
			}
			if ($status < 200 || $status > 299) {
			throw new JO_Api_Rackspace_Exception_Invalidresponse(
			"Invalid response: ".$this->cfs_http->get_error());
			}
			return new JO_Api_Rackspace_Container($this->cfs_auth, $this->cfs_http,
			...
			*
			* Return an array of JO_Api_Rackspace_Container instances on the account.  The instances
			...
			* </code>
			*
			* @return array An array of JO_Api_Rackspace_Container instances
			* @throws JO_Api_Rackspace_Exception_Invalidresponse unexpected response
			*/
			...
			$containers[] = new JO_Api_Rackspace_Container($this->cfs_auth, $this->cfs_http,
						...
						*         (
							...
							* </code>
								...
								* function read_callback($bytes_transferred) {
									...
									* the JO_Api_Rackspace_Authentication instance has been instantiated.
									*
									...
									*/
									...
									#{
									#    $new_auth = new JO_Api_Rackspace_Authentication(
