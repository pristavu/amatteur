...
class JO_Url_Relativetoabsolute {
	...
	public static function toAbsolute( $baseUrl, $relativeUrl ) {
		...
		if ( $r['path'][0] != '/' )
		{
			$base = mb_strrchr( ( isset($b['path']) ? $b['path'] : '' ), '/', TRUE, 'UTF-8' );
			if ( $base === FALSE ) $base = '';
			$r['path'] = $base . '/' . $r['path'];
		}
		$r['path'] = self::url_remove_dot_segments( $r['path'] );
	 ...
	 */
	private static function split_url( $url, $decode=FALSE)
	{
	 ...
	 */
	private static function join_url( $parts, $encode=FALSE)
	{
		if ( $encode )
		{
			if ( isset( $parts['user'] ) )
