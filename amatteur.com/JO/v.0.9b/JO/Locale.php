...
class JO_Locale
{
     ...
     *
     * @param  string|JO_Locale $locale (Optional) Locale for parsing input
     * @throws JO_Locale_Exception When autodetection has been failed
     */
    ...
    public static function getDefault()
    {
        if ((self::$compatibilityMode === true) or (func_num_args() > 0)) {
            if (!self::$_breakChain) {
                ...
                trigger_error('You are running JO_Locale in compatibility mode... please migrate your scripts', E_USER_NOTICE);
     ...
     *
     * @param  string|JO_Locale $locale  Locale to set
     * @param  float              $quality The quality to set from 0 to 1
     * @throws JO_Locale_Exception When a autolocale was given
     * @throws JO_Locale_Exception When a unknown locale was given
     * @return void
     */
    public static function setDefault($locale, $quality = 1)
    {
        if (($locale === 'auto') or ($locale === 'root') or ($locale === 'default') or
            ...
            throw new JO_Exception('Only full qualified locales can be used as default!');
        }

        if (($quality < 0.1) or ($quality > 100)) {
            require_once 'JO/Exception.php';
            throw new JO_Exception("Quality must be between 0.1 and 100");
        }
        ...
        if (isset(self::$_localeData[(string) $locale]) === true) {
            ...
            if (isset(self::$_localeData[$elocale[0]]) === true) {
                ...
                require_once 'JO/Exception.php';
                throw new JO_Exception("Unknown locale '" . (string) $locale . "' can not be set as default!");
            }
        }
     ...
     * Sets a new locale
     *
     * @param  string|JO_Locale $locale (Optional) New locale to set
     * @return void
     */
     ...
     *
     * @param  JO_Locale $object Locale to check for equality
     * @return boolean
     */
    public function equals(JO_Locale $object)
    {
     ...
     *
     * @param  string|JO_Locale $locale     Locale to check for
	...
	public static function listTerritory()
    {
        return self::$_territoryData;
    }
