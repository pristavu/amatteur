 ...
 * @since      2008-03-13
 */
class JO_Phpthumb_Factory_Imagick_Phmagick_Plugins_Decorations {
    function roundCorners(JO_Phpthumb_Factory_Imagick_Phmagick $p,$i = 15){

        //original idea from Leif ï¿½ï¿½strand <leif@sitelogic.fi>
    ...
    function dropShadow(JO_Phpthumb_Factory_Imagick_Phmagick $p,$color = '#000', $offset = 4, $transparency = 60, $top = 4, $left=4){
     ...
     */
    function fakePolaroid(JO_Phpthumb_Factory_Imagick_Phmagick $p,$rotate = 6 , $borderColor = "#fff", $background ="none"){
        $cmd = $p->getBinary('convert');
        $cmd .= ' "' . $p->getSource().'"'  ;
     ...
     * @param $background - Image background color (use for jpegs or images that do not support transparency or you will end up with a black background)
     */
    function polaroid(JO_Phpthumb_Factory_Imagick_Phmagick $p, $format = null, $rotation= 6, $borderColor="snow", $shaddowColor = "black", $background="none"){
        ...
        if (get_class($format) == 'phMagickTextObject' ){
            ...
            $tmp = new JO_Phpthumb_Factory_Imagick_Phmagick_Textobject();
            $tmp->text($format);
            $format = $tmp ;
        }
