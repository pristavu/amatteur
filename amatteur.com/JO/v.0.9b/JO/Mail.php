...
class JO_Mail {
    ...
    */
    ...
    /**
    * An array of embedded files   /objects
    * @private array
    */
    private $html_files;
    /**
    ...
    */
    public function setSMTPParams($host = null, $port = null, $user = null, $pass = null, $helo = null, $auth = null)
    {
    ...
    */
    public function setTextEncoding(JO_Mail_IEncoding $encoding)
    {
    ...
    */
    public function setHTMLEncoding(JO_Mail_IEncoding $encoding)
    {
    ...
    */
    function setHTML($html, $images_dir = '')
    {
        $this->html = $html;
    ...
    */
    private function findHtmlImages($images_dir)
    {
        ...
        if( preg_match_all('/(?:"|\')([^"\']+\.('.implode('|', $extensions).'))(?:"|\')/Ui', $this->html, $matches) ) {

	        foreach ($matches[1] as $m) {
	            if (file_exists($images_dir . $m) || @file_get_contents($images_dir . $m)) {
	                $html_images[] = $m;
        ...
        */
        if (!empty($html_images)) {
            ...
            foreach ($html_images as $img) {
                if ( ($image = @file_get_contents($images_dir . $img)) !== false ) {
                    $ext          = preg_replace('#^.*\.(\w{3,4})$#e', 'strtolower("$1")', $img);
                    $content_type = $this->image_types[$ext];
                    $this->addEmbeddedImage(new JO_Mail_StringEmbeddedImage($image, basename($img), $content_type));
                }
            }
        }
    }
    ...
    /**
    * Adds an file to the list of embedded
    * files.
    * 
     * @param JO_Mail_StringEmbeddedFile $embeddedFile
     */
    public function addEmbeddedFile(JO_Mail_StringEmbeddedFile $embeddedFile)
    {
        $embeddedImage->cid = md5(uniqid(time()));

        $this->html_files[] = $embeddedFile;
    }
    ...
    * images.
    * 
    * @param JO_Mail_StringEmbeddedImage $embeddedImage
    */
    public function addEmbeddedImage(JO_Mail_StringEmbeddedImage $embeddedImage)
    {
        $embeddedImage->cid = md5(uniqid(time()));
    ...
    */
    private function addTextPart(&$message)
    {
        ...
        if (!empty($message)) {
            ...
            $message = new JO_Mail_MimePart($this->text, $params);
        }
    }
    ...
    */
    private function addHtmlPart(&$message)
    {
        ...
        if (!empty($message)) {
            ...
            $message = new JO_Mail_MimePart($this->html, $params);
        }
    }
    ...
    */
    private function addMixedPart(&$message)
    {
        ...
        $message = new JO_Mail_MimePart('', $params);
    }
    ...
    * @param object $obj Message object
    */
    private function addAttachmentFiles(&$message)
    {
        foreach ($this->html_files as $value) {
            $params['content_type'] = $value->contentType;
            $params['encoding']     = $value->encoding->getType();
    ...
    */
    private function build()
    {
        ...
        $html_files  = !empty($this->html_files);
        ...
        switch (true) {
            ...
            case $html AND !$attachments AND !$html_images AND !$html_files:
            ...
            case $html AND !$attachments AND !$html_images AND $html_files:
                ...
                // Attachments Files
                $this->addAttachmentFiles($message);
                break;
