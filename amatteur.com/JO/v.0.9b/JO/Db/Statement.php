...
/**
 * @see JO_Db_Statement_Interface
 */
require_once 'JO/Db/Statement/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class JO_Db_Statement implements JO_Db_Statement_Interface
{
     ...
     * @param mixed $sql Either a string or JO_Db_Select.
     */
        ...
        foreach ($this->_sqlSplit as $key => $val) {
            if ($val == '?') {
                if ($this->_adapter->supportsParameters('positional') === false) {
                    ...
                    throw new JO_Db_Statement_Exception("Invalid bind-variable position '$val'");
                }
            } else if ($val[0] == ':') {
                if ($this->_adapter->supportsParameters('named') === false) {
                     ...
                     */
                    require_once 'JO/Db/Statement/Exception.php';
                    throw new JO_Db_Statement_Exception("Invalid bind-variable name '$val'");
                }
            }
            $this->_sqlParam[] = $val;
        }
    ...
    public function bindParam($parameter, &$variable, $type = null, $length = null, $options = null)
    {
        if (!is_int($parameter) && !is_string($parameter)) {
             ...
             */
            require_once 'JO/Db/Statement/Exception.php';
            throw new JO_Db_Statement_Exception('Invalid bind-variable position');
        }
        ...
        if ($position === null) {
             ...
             */
            require_once 'JO/Db/Statement/Exception.php';
            throw new JO_Db_Statement_Exception("Invalid bind-variable position '$parameter'");
        }
    ...
    {
        $data = array();
        if ($style === JO_Db::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        $data = array();
        $col = (int) $col;
        $row = $this->fetch(JO_Db::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(JO_Db::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
     ...
     * @return bool
     * @throws JO_Db_Statement_Exception
     */
    public function setFetchMode($mode)
    {
        switch ($mode) {
            ...
            default:
                 ...
                 */
                require_once 'JO/Db/Statement/Exception.php';
                throw new JO_Db_Statement_Exception('invalid fetch mode');
                break;
        }
    }
     ...
     * particular JO_Db_Statement object.
     *
