 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class JO_Db_Table_Row_Abstract implements ArrayAccess, IteratorAggregate
{
    ...
    /**
     * JO_Db_Table_Abstract parent class or instance.
     *
     ...
     * the database schema (e.g. evaluated columns/JO_Db_Expr columns). This can also be passed
    ...
    /**
     * Name of the class of the JO_Db_Table_Abstract object.
     *
     * @var string
     */
     ...
     * - table       = class name or object of type JO_Db_Table_Abstract
    ...
    {
        if (isset($config['table']) && $config['table'] instanceof JO_Db_Table_Abstract) {
        ...
        if (isset($config['data'])) {
            if (!is_array($config['data'])) {
                ...
                throw new JO_Db_Table_Row_Exception('Data must be an array');
            }
     ...
     * @throws JO_Db_Table_Row_Exception if the $columnName is not a string.
     */
    protected function _transformColumn($columnName)
    {
        if (!is_string($columnName)) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception('Specified column is not a string');
        }
     ...
     * @throws JO_Db_Table_Row_Exception if the $columnName is not a column in the row.
     */
    public function __get($columnName)
    {
        ...
        if (!array_key_exists($columnName, $this->_data)) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Specified column \"$columnName\" is not in the row");
        }
    ...
    public function __unset($columnName)
    {
        ...
        if ($this->isConnected() && in_array($columnName, $this->_table->info('primary'))) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Specified column \"$columnName\" is a primary key and should not be unset");
        }
     ...
     *
     * @return JO_Db_Table_Abstract|null
     */
    ...
    /**
     * Returns an instance of the parent table's JO_Db_Table_Select object.
     *
    ...
    protected function _doInsert()
    {
        ...
        if ($this->_readOnly === true) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception('This row has been marked read-only');
        }
     ...
     *
     * @param  array $data
     * @return JO_Db_Table_Row_Abstract Provides a fluent interface
     */
    ...
    protected function _getTable()
    {
        if (!$this->_connected) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception('Cannot save a Row unless it is connected');
        }
        return $this->_table;
    }
    ...
    protected function _getPrimaryKey($useDirty = true)
    {
        if (!is_array($this->_primary)) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("The primary key must be set as an array");
        }
        ...
        if (count($primary) != count($array)) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("The specified Table '$this->_tableClass' does not have the same primary key as the Row");
        }
        return $array;
    }
    ...
    protected function _getWhereQuery($useDirty = true)
    {
        ...
        $metadata = $info[JO_Db_Table_Abstract::METADATA];

        // retrieve recently updated row using primary keys
        $where = array();
        foreach ($primaryKey as $column => $value) {
            $tableName = $db->quoteIdentifier($info[JO_Db_Table_Abstract::NAME], true);
    ...
    protected function _refresh()
    {
        ...
        if (null === $row) {
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception('Cannot refresh row as parent is missing');
        }
     ...
     *
     * @param JO_Db_Table_Abstract $dependentTable
     * @param JO_Db_Table_Abstract $parentTable
     * @param string                 $ruleKey
     * @return array
     */
    protected function _prepareReference(JO_Db_Table_Abstract $dependentTable, JO_Db_Table_Abstract $parentTable, $ruleKey)
    {
        $parentTableName = (get_class($parentTable) === 'JO_Db_Table') ? $parentTable->getDefinitionConfigName() : get_class($parentTable);
        $map = $dependentTable->getReference($parentTableName, $ruleKey);

        if (!isset($map[JO_Db_Table_Abstract::REF_COLUMNS])) {
            $parentInfo = $parentTable->info();
            $map[JO_Db_Table_Abstract::REF_COLUMNS] = array_values((array) $parentInfo['primary']);
        }

        $map[JO_Db_Table_Abstract::COLUMNS] = (array) $map[JO_Db_Table_Abstract::COLUMNS];
        $map[JO_Db_Table_Abstract::REF_COLUMNS] = (array) $map[JO_Db_Table_Abstract::REF_COLUMNS];
     ...
     *
     * @param string|JO_Db_Table_Abstract  $dependentTable
     * @param string                         OPTIONAL $ruleKey
     * @param JO_Db_Table_Select           OPTIONAL $select
     * @return JO_Db_Table_Rowset_Abstract Query result from $dependentTable
     * @throws JO_Db_Table_Row_Exception If $dependentTable is not a table or is not loadable.
     */
    public function findDependentRowset($dependentTable, $ruleKey = null, JO_Db_Table_Select $select = null)
    {
        $db = $this->_getTable()->getAdapter();
        ...
        if (!$dependentTable instanceof JO_Db_Table_Abstract) {
            $type = gettype($dependentTable);
            if ($type == 'object') {
                $type = get_class($dependentTable);
            }
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Dependent table must be a JO_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $dependentTable->setOptions(array(JO_Db_Table_Abstract::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($map[JO_Db_Table_Abstract::COLUMNS]); ++$i) {
            $parentColumnName = $db->foldCase($map[JO_Db_Table_Abstract::REF_COLUMNS][$i]);
            ...
            $dependentColumnName = $dependentDb->foldCase($map[JO_Db_Table_Abstract::COLUMNS][$i]);
            $dependentColumn = $dependentDb->quoteIdentifier($dependentColumnName, true);
            $dependentInfo = $dependentTable->info();
            $type = $dependentInfo[JO_Db_Table_Abstract::METADATA][$dependentColumnName]['DATA_TYPE'];
     ...
     *
     * @param string|JO_Db_Table_Abstract $parentTable
     * @param string                        OPTIONAL $ruleKey
     * @param JO_Db_Table_Select          OPTIONAL $select
     * @return JO_Db_Table_Row_Abstract   Query result from $parentTable
     * @throws JO_Db_Table_Row_Exception If $parentTable is not a table or is not loadable.
     */
    public function findParentRow($parentTable, $ruleKey = null, JO_Db_Table_Select $select = null)
    {
        $db = $this->_getTable()->getAdapter();
        ...
        if (!$parentTable instanceof JO_Db_Table_Abstract) {
            $type = gettype($parentTable);
            if ($type == 'object') {
                $type = get_class($parentTable);
            }
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Parent table must be a JO_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $parentTable->setOptions(array(JO_Db_Table_Abstract::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($map[JO_Db_Table_Abstract::COLUMNS]); ++$i) {
            $dependentColumnName = $db->foldCase($map[JO_Db_Table_Abstract::COLUMNS][$i]);
            ...
            $parentColumnName = $parentDb->foldCase($map[JO_Db_Table_Abstract::REF_COLUMNS][$i]);
            $parentColumn = $parentDb->quoteIdentifier($parentColumnName, true);
            $parentInfo = $parentTable->info();

            // determine where part
            $type     = $parentInfo[JO_Db_Table_Abstract::METADATA][$parentColumnName]['DATA_TYPE'];
            $nullable = $parentInfo[JO_Db_Table_Abstract::METADATA][$parentColumnName]['NULLABLE'];
    ...
    /**
     * @param  string|JO_Db_Table_Abstract  $matchTable
     * @param  string|JO_Db_Table_Abstract  $intersectionTable
     * @param  string                         OPTIONAL $callerRefRule
     * @param  string                         OPTIONAL $matchRefRule
     * @param  JO_Db_Table_Select           OPTIONAL $select
     * @return JO_Db_Table_Rowset_Abstract Query result from $matchTable
     * @throws JO_Db_Table_Row_Exception If $matchTable or $intersectionTable is not a table class or is not loadable.
     */
    public function findManyToManyRowset($matchTable, $intersectionTable, $callerRefRule = null,
                                         $matchRefRule = null, JO_Db_Table_Select $select = null)
    {
        $db = $this->_getTable()->getAdapter();
        ...
        if (!$intersectionTable instanceof JO_Db_Table_Abstract) {
            $type = gettype($intersectionTable);
            if ($type == 'object') {
                $type = get_class($intersectionTable);
            }
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Intersection table must be a JO_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $intersectionTable->setOptions(array(JO_Db_Table_Abstract::DEFINITION => $tableDefinition));
        }
        ...
        if (! $matchTable instanceof JO_Db_Table_Abstract) {
            $type = gettype($matchTable);
            if ($type == 'object') {
                $type = get_class($matchTable);
            }
            require_once 'JO/Db/Table/Row/Exception.php';
            throw new JO_Db_Table_Row_Exception("Match table must be a JO_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $matchTable->setOptions(array(JO_Db_Table_Abstract::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($matchMap[JO_Db_Table_Abstract::COLUMNS]); ++$i) {
            $interCol = $interDb->quoteIdentifier('i' . '.' . $matchMap[JO_Db_Table_Abstract::COLUMNS][$i], true);
            $matchCol = $interDb->quoteIdentifier('m' . '.' . $matchMap[JO_Db_Table_Abstract::REF_COLUMNS][$i], true);
            $joinCond[] = "$interCol = $matchCol";
        }
        $joinCond = implode(' AND ', $joinCond);

        $select->from(array('i' => $interName), array(), $interSchema)
               ->joinInner(array('m' => $matchName), $joinCond, JO_Db_Select::SQL_WILDCARD, $matchSchema)
               ->setIntegrityCheck(false);

        $callerMap = $this->_prepareReference($intersectionTable, $this->_getTable(), $callerRefRule);

        for ($i = 0; $i < count($callerMap[JO_Db_Table_Abstract::COLUMNS]); ++$i) {
            $callerColumnName = $db->foldCase($callerMap[JO_Db_Table_Abstract::REF_COLUMNS][$i]);
            $value = $this->_data[$callerColumnName];
            $interColumnName = $interDb->foldCase($callerMap[JO_Db_Table_Abstract::COLUMNS][$i]);
            $interCol = $interDb->quoteIdentifier("i.$interColumnName", true);
            $interInfo = $intersectionTable->info();
            $type = $interInfo[JO_Db_Table_Abstract::METADATA][$interColumnName]['DATA_TYPE'];
     ...
     *
     * @param string $method
     * @param array $args OPTIONAL JO_Db_Table_Select query modifier
     * @return JO_Db_Table_Row_Abstract|JO_Db_Table_Rowset_Abstract
     * @throws JO_Db_Table_Row_Exception If an invalid method is called.
     */
    public function __call($method, array $args)
    {
        $matches = array();

        if (count($args) && $args[0] instanceof JO_Db_Table_Select) {
            $select = $args[0];
        } else {
            $select = null;
        }
        ...
        require_once 'JO/Db/Table/Row/Exception.php';
        throw new JO_Db_Table_Row_Exception("Unrecognized method '$method()'");
    }
