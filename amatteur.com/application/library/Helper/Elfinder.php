...
class Helper_Elfinder {
	...
	protected function _isUploadAllow($name, $tmpName)
	{
		$allow = false;
		$deny  = false;
		$mime  = $this->_mimetype( !in_array($this->_options['mimeDetect'], array( 'internal', 'jo' )) ? $tmpName : $name);
	...
	protected function _path2url($path)
	{
		...
		return $this->_options['URL'].($dir ? str_replace(DIRECTORY_SEPARATOR, '/', $dir).'/' : '').$file . '?' . microtime(true);
	}
